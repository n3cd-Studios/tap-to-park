...
export class Paginator<T> {
  ...
  load = async () => {
    const payload = await get<AsPaginated<T>>({
      ...this.params,
      params: {
        ...this.params.params,
        page: `${this.page}`,
...
const getHelper = <T, R>(
  { route, params, body, headers, method = "GET" }: GetParams,
  defaultValue: R,
): Promise<T | R> =>
  fetch(
 ...
 */
export const getWithDefault = <T>(params: GetParams, defaultValue: T) =>
  getHelper<T, T>(params, defaultValue);
